...
try {
    $stmt_master = $pdo->prepare("SELECT id_jenis_file, nama_jenis_file FROM masterfilelampiran WHERE id_jenis_file >= 9 AND id_jenis_file != 15 ORDER BY id_jenis_file");
    $stmt_master->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        // Simpan ke tabel datausaha
        $nama_usaha = trim($_POST['nama_usaha']);
        $jenis_pemohon = trim($_POST['jenis_pemohon']);
        ...
        $stmt = $pdo->prepare("INSERT INTO datausaha (nama_usaha, rt_rw, kel_desa, kecamatan, no_telp_perusahaan, hasil_produk, jml_tenaga_kerja, kapasitas_produk, omset_perbulan, wilayah_pemasaran, jenis_pemohon)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$nama_usaha, $rt_rw, $kel_desa, $kecamatan, $no_telp_perusahaan, $hasil_produk_str, $jml_tenaga_kerja, $kapasitas_produk_str, $omset_perbulan_str, $wilayah_pemasaran, $jenis_pemohon]);
        ...
        function uploadMultipleFiles($inputName, $id_jenis_file, $id_pendaftaran, $pdo, $NIK, $fileType, $isRequired = false)
        {
            ...
            switch ($id_jenis_file) {
                ...
                case 13: // SNI
                    ...
                    break;
                case 15: // Akta Pendirian
                    $baseFolder = "uploads/akta/akta_{$NIK}/";
                    break;
                default:
        ...
        // Upload lampiran legalitas
        if (isset($_FILES)) {
            foreach ($_FILES as $inputName => $fileData) {
                // Cek apakah ini file legalitas
        ...
        // Upload lampiran CUSTOM legalitas
        if (isset($_FILES)) {
            $customIndex = 0;
            foreach ($_FILES as $inputName => $fileData) {
                if (preg_match('/^custom_legalitas_file_(\d+)$/', $inputName, $matches)) {
                    $index = $matches[1];
                    $customName = isset($_POST["custom_legalitas_name_{$index}"]) ? trim($_POST["custom_legalitas_name_{$index}"]) : '';

                    if (empty($customName)) {
                        throw new Exception("Nama legalitas custom tidak boleh kosong");
                    }

                    $stmt_insert = $pdo->prepare("INSERT INTO masterfilelampiran (nama_jenis_file) VALUES (?)");
                    $stmt_insert->execute([$customName]);
                    $new_id_jenis_file = $pdo->lastInsertId();

                    $folderName = preg_replace('/[^a-zA-Z0-9]/', '', $customName);
                    $baseFolder = "uploads/legalitas/{$folderName}_{$NIK}/";
                    ...
                    if (!empty($fileData['name'])) {
                        ...
                        if (!in_array($file_extension, $allowed_extensions)) {
                            throw new Exception("Format file tidak diizinkan: {$fileData['name']}");
                        }

                        if ($fileData['size'] > 1 * 1024 * 1024) {
                            throw new Exception("Ukuran file {$fileData['name']} melebihi 1 MB");
                        }
                        ...
                        if (move_uploaded_file($fileData['tmp_name'], $target)) {
                            ...
                            $stmt = $pdo->prepare("INSERT INTO lampiran (id_pendaftaran, id_jenis_file, tgl_upload, file_path)
                                                  VALUES (?, ?, ?, ?)");
                            $stmt->execute([$id_pendaftaran, $new_id_jenis_file, $tgl_upload, $target]);
                        } else {
                            throw new Exception("Gagal mengupload file {$fileData['name']}");
                        }
                    }
                }
            }
        ...
        // Upload Akta Pendirian jika ada
        uploadMultipleFiles('akta_files', 15, $id_pendaftaran, $pdo, $NIK, 'akta', false);
...
<body>
                    ...
                    <form method="POST" enctype="multipart/form-data" id="formPendaftaran">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control" id="rt_rw" name="rt_rw" placeholder="Contoh: 002/006" required>
                            </div>
                            <div class="col-md-6 mb-3">
                        ...
                        <!-- Upload Akta Pendirian CV/PT -->
                        <div class="mb-3" id="aktaWrapper" style="display: none;">
                            <label class="form-label">Akta Pendirian CV/PT <span class="text-danger" id="aktaRequired">*</span></label>
                            <div class="alert alert-info" style="font-size: 0.85rem; border-left: 4px solid #0d6efd; background-color: rgba(13, 110, 253, 0.05);">
                                ...
                                <ul class="mb-0 mt-2" style="padding-left: 20px;">
                                    <li>Wajib upload <strong>Akta Pendirian</strong> jika pendaftaran merek menggunakan atas nama <strong>Perusahaan (CV/PT)</strong></li>
                                </ul>
                            </div>
                            <div class="file-drop-zone" id="aktaDropZone">
                                <i class="fas fa-cloud-upload-alt"></i>
                                <p><strong>Seret & Lepas file di sini</strong><br>atau klik untuk memilih file</p>
                                <small>Upload maksimal 3 file PDF. Maks 5 MB per file</small>
                                <input type="file" name="akta_files[]" id="akta-file" accept=".pdf" multiple hidden>
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <p class="text-muted small">Upload file legalitas yang Anda miliki, kemudian pilih jenis legalitasnya dari dropdown<br>
                                <span style="font-weight: 600;">Contohnya: P-IRT, BPOM-MD, HALAL, NUTRITION FACTS, SNI, dll.</span>
                            </p>

                            <div class="alert alert-secondary" style="font-size: 0.85rem;">
                                <i class="bi bi-info-circle me-2"></i>
                                <strong>Catatan:</strong> Jika jenis legalitas tidak ada di dropdown, pilih <strong>"Lainnya"</strong> dan ketik nama legalitasnya.
                            </div>
    ...
    <script>
        ...
        console.log('Master Legalitas Loaded:', masterLegalitas);
    </script>
